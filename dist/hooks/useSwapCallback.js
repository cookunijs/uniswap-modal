...
const v1SwapArguments_1 = __importDefault(require("../utils/v1SwapArguments"));
...
recipientAddressOrName // the ENS name or address of the recipient of the trade, or null if swap should be returned to sender
    ...
    const { account, chainId, library } = index_1.useActiveWeb3React();
    const { address: recipientAddress } = useENS_1.default(recipientAddressOrName);
    const recipient = recipientAddressOrName === null ? account : recipientAddress;
    const deadline = useTransactionDeadline_1.default();
    const v1Exchange = useContract_1.useV1ExchangeContract(V1_1.useV1TradeExchangeAddress(trade), true);
    return react_1.useMemo(() => {
        const tradeVersion = V1_1.getTradeVersion(trade);
        if (!trade || !recipient || !library || !account || !tradeVersion || !chainId || !deadline)
            return [];
        const contract = tradeVersion === useToggledVersion_1.Version.v2 ? utils_1.getRouterContract(chainId, library, account) : v1Exchange;
        if (!contract) {
            return [];
        }
        const swapMethods = [];
            ...
            callback: function onSwap() {
                return __awaiter(this, void 0, void 0, function* () {
                    const estimatedCalls = yield Promise.all(swapCalls.map(call => {
                        ...
                        const options = !value || isZero_1.default(value) ? {} : { value };
                    ...
                    return contract[methodName](...args, Object.assign({ gasLimit: utils_1.calculateGasMargin(gasEstimate) }, (value && !isZero_1.default(value) ? { value, from: account } : { from: account })))
                        .then((response) => {
                        ...
                        const withVersion = tradeVersion === useToggledVersion_1.Version.v2 ? withRecipient : `${withRecipient} on ${tradeVersion.toUpperCase()}`;
