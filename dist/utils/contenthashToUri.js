...
function contenthashToUri(contenthash) {
    ...
    const codec = multicodec_1.getCodec(buff); // the typing is wrong for @types/multicodec
    switch (codec) {
        case 'ipfs-ns': {
            const data = multicodec_1.rmPrefix(buff);
        ...
        case 'ipns-ns': {
            const data = multicodec_1.rmPrefix(buff);
            const cid = new cids_1.default(data);
            const multihash = multihashes_1.decode(cid.multihash);
