...
const initialState = {
    lastInitializedDefaultListOfLists: lists_2.DEFAULT_LIST_OF_LISTS,
    byUrl: Object.assign({}, lists_2.DEFAULT_LIST_OF_LISTS.reduce((memo, listUrl) => {
        memo[listUrl] = NEW_LIST_STATE;
        return memo;
    }, {})),
    activeListUrls: lists_1.DEFAULT_ACTIVE_LIST_URLS
};
exports.default = toolkit_1.createReducer(initialState, builder => builder
    .addCase(actions_2.fetchTokenList.pending, (state, { payload: { requestId, url } }) => {
    state.byUrl[url] = Object.assign(Object.assign({ current: null, pendingUpdate: null }, state.byUrl[url]), { loadingRequestId: requestId, error: null });
})
    ...
    if (current) {
        const upgradeType = token_lists_1.getVersionUpgrade(current.version, tokenList.version);
        ...
        if (lists_1.DEFAULT_ACTIVE_LIST_URLS.includes(url)) {
    ...
    if (!state.lastInitializedDefaultListOfLists) {
        ...
        const newListOfListsSet = lists_2.DEFAULT_LIST_OF_LISTS.reduce((s, l) => s.add(l), new Set());
        lists_2.DEFAULT_LIST_OF_LISTS.forEach(listUrl => {
    ...
    state.lastInitializedDefaultListOfLists = lists_2.DEFAULT_LIST_OF_LISTS;
    // if no active lists, activate defaults
    if (!state.activeListUrls) {
        state.activeListUrls = lists_1.DEFAULT_ACTIVE_LIST_URLS;
        // for each list on default list, initialize if needed
        lists_1.DEFAULT_ACTIVE_LIST_URLS.map((listUrl) => {
