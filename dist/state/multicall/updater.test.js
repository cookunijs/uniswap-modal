...
describe('multicall updater', () => {
    describe('#activeListeningKeys', () => {
        it('ignores 0, returns call key to block age key', () => {
            expect(updater_1.activeListeningKeys({
                [1]: {
                    ['abc']: {
                        4: 2,
    ...
    describe('#outdatedListeningKeys', () => {
        ...
        it('returns only outdated keys', () => {
            expect(updater_1.outdatedListeningKeys({ [1]: { abc: { data: '0x', blockNumber: 2 } } }, { abc: 1, def: 1 }, 1, 2)).toEqual(['def']);
        });
        it('returns only keys not being fetched', () => {
            expect(updater_1.outdatedListeningKeys({
                [1]: { abc: { data: '0x', blockNumber: 2 }, def: { fetchingBlockNumber: 2 } }
            }, { abc: 1, def: 1 }, 1, 2)).toEqual([]);
        });
        it('returns keys being fetched for old blocks', () => {
            expect(updater_1.outdatedListeningKeys({ [1]: { abc: { data: '0x', blockNumber: 2 }, def: { fetchingBlockNumber: 1 } } }, { abc: 1, def: 1 }, 1, 2)).toEqual(['def']);
        });
        it('respects blocks per fetch', () => {
            expect(updater_1.outdatedListeningKeys({ [1]: { abc: { data: '0x', blockNumber: 2 }, def: { data: '0x', fetchingBlockNumber: 1 } } }, { abc: 2, def: 2 }, 1, 3)).toEqual(['def']);
        });
    });
});
