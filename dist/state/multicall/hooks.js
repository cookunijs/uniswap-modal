...
function useCallsData(calls, options) {
    ...
    const callResults = react_redux_1.useSelector(state => state.multicall.callResults);
    const dispatch = react_redux_1.useDispatch();
    const serializedCallKeys = react_1.useMemo(() => {
        var _a, _b, _c;
    ...
    react_1.useEffect(() => {
        ...
        if (!chainId || callKeys.length === 0)
            return undefined;
        const calls = callKeys.map(key => actions_1.parseCallKey(key));
    ...
    return react_1.useMemo(() => calls.map(call => {
        var _a;
        if (!chainId || !call)
            return INVALID_RESULT;
        const result = (_a = callResults[chainId]) === null || _a === void 0 ? void 0 : _a[actions_1.toCallKey(call)];
        let data;
...
function useSingleContractMultipleData(contract, methodName, callInputs, options) {
    const fragment = react_1.useMemo(() => { var _a; return (_a = contract === null || contract === void 0 ? void 0 : contract.interface) === null || _a === void 0 ? void 0 : _a.getFunction(methodName); }, [contract, methodName]);
    const calls = react_1.useMemo(() => contract && fragment && callInputs && callInputs.length > 0
    ...
    const latestBlockNumber = hooks_2.useBlockNumber();
...
function useMultipleContractSingleData(addresses, contractInterface, methodName, callInputs, options) {
    const fragment = react_1.useMemo(() => contractInterface.getFunction(methodName), [contractInterface, methodName]);
    const callData = react_1.useMemo(() => fragment && isValidMethodArgs(callInputs)
        ? contractInterface.encodeFunctionData(fragment, callInputs)
        : undefined, [callInputs, contractInterface, fragment]);
    const calls = react_1.useMemo(() => fragment && addresses && addresses.length > 0 && callData
