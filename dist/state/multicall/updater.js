...
function Updater() {
    ...
    const state = react_redux_1.useSelector(state => state.multicall);
    // wait for listeners to settle before triggering updates
    const debouncedListeners = useDebounce_1.default(state.callListeners, 100);
    ...
    const cancellations = react_1.useRef();
    const listeningKeys = react_1.useMemo(() => {
        return activeListeningKeys(debouncedListeners, chainId);
    }, [debouncedListeners, chainId]);
    const unserializedOutdatedCallKeys = react_1.useMemo(() => {
        return outdatedListeningKeys(state.callResults, listeningKeys, chainId, latestBlockNumber);
    }, [chainId, state.callResults, listeningKeys, latestBlockNumber]);
    const serializedOutdatedCallKeys = react_1.useMemo(() => JSON.stringify(unserializedOutdatedCallKeys.sort()), [
    ...
    react_1.useEffect(() => {
        ...
        if (outdatedCallKeys.length === 0)
            return;
        const calls = outdatedCallKeys.map(key => actions_1.parseCallKey(key));
        const chunkedCalls = chunkArray_1.default(calls, CALL_CHUNK_SIZE);
        ...
        cancellations.current = {
            ...
            cancellations: chunkedCalls.map((chunk, index) => {
                const { cancel, promise } = retry_1.retry(() => fetchChunk(multicallContract, chunk, latestBlockNumber), {
                    n: Infinity,
                    minWait: 2500,
                    maxWait: 3500
                });
