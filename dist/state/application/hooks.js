...
function useBlockNumber() {
    ...
    return react_redux_1.useSelector((state) => state.application.blockNumber[chainId !== null && chainId !== void 0 ? chainId : -1]);
}
exports.useBlockNumber = useBlockNumber;
function useModalOpen(modal) {
    const openModal = react_redux_1.useSelector((state) => state.application.openModal);
...
function useToggleModal(modal) {
    ...
    return react_1.useCallback(() => dispatch(actions_1.setOpenModal(open ? null : modal)), [dispatch, modal, open]);
}
exports.useToggleModal = useToggleModal;
function useOpenModal(modal) {
    const dispatch = react_redux_1.useDispatch();
    return react_1.useCallback(() => dispatch(actions_1.setOpenModal(modal)), [dispatch, modal]);
}
exports.useOpenModal = useOpenModal;
function useCloseModals() {
    const dispatch = react_redux_1.useDispatch();
    return react_1.useCallback(() => dispatch(actions_1.setOpenModal(null)), [dispatch]);
}
exports.useCloseModals = useCloseModals;
function useWalletModalToggle() {
    return useToggleModal(actions_1.ApplicationModal.WALLET);
}
...
function useAddPopup() {
    const dispatch = react_redux_1.useDispatch();
    return react_1.useCallback((content, key) => {
        dispatch(actions_1.addPopup({ content, key }));
    }, [dispatch]);
}
...
function useRemovePopup() {
    const dispatch = react_redux_1.useDispatch();
    return react_1.useCallback((key) => {
        dispatch(actions_1.removePopup({ key }));
    }, [dispatch]);
}
exports.useRemovePopup = useRemovePopup;
// get the list of active popups
function useActivePopups() {
    const list = react_redux_1.useSelector((state) => state.application.popupList);
    return react_1.useMemo(() => list.filter(item => item.show), [list]);
}
