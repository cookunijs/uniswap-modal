...
function useTransactionAdder() {
    ...
    return react_1.useCallback((response, { summary, approval, claim } = {}) => {
        if (!account)
            return;
        if (!chainId)
            return;
        ...
        dispatch(actions_1.addTransaction({ hash, from: account, chainId, approval, summary, claim }));
...
function useHasPendingApproval(tokenAddress, spender) {
    ...
    return react_1.useMemo(() => typeof tokenAddress === 'string' &&
...
function useUserHasSubmittedClaim(account) {
    ...
    const claimTxn = react_1.useMemo(() => {
